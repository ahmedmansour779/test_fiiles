...
class ProductsController extends ControllerHelper
{
    ...
    public function allProducts(Request $request)
    {
        try {
            ...
            if ($lang) {
                $query = $query->leftJoin('product_langs as b', function ($join) use ($lang) {
                ...
                $query = $query->select('products.id', 'b.title');

            } else {
                if ($request->q) {
                    $query = $query->where('products.title', 'LIKE', "%{$request->q}%");
                }
                $query = $query->select('products.id', 'products.title');
            }

            $query = $query->orderBy('products.created_at');
            if($request->per_page) {
    ...
    public function delete(Request $request, $id)
    {
        try {
            ...
            foreach ($ids as $i) {
                ...
                OfferProduct::where('product_id', $i)->delete();
