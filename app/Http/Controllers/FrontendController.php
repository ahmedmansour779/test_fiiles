...
class FrontendController extends Controller
{
    public function all(Request $request)
    {
        try {
            ...
            if ($lang) {
                ...
                $query = Utils::offerQueryLang($request, $query,$lang);
                ...
                $query = $query->select('products.id', 'products.slug', 'pl.title', 'products.badge',
                    'products.selling', 'products.offered', 'products.brand_id', 'products.shipping_rule_id',
                    'products.image', 'products.review_count', 'products.rating', 'flash_sale_products.price',
                    'flash_sales.end_time', 'products.admin_id');
                ...
                $query = Utils::offerQuery($request, $query);
                ...
                $query = $query->select('products.id', 'products.title', 'products.slug',
                    'products.badge', 'products.admin_id',
                    'products.selling', 'products.offered', 'products.brand_id', 'products.shipping_rule_id',
                    'products.image', 'products.review_count', 'products.rating', 'flash_sale_products.price',
        ...
        try {
            ...
            if ($lang) {
                ...
                $query = $query->select('products.id', 'products.title', 'products.slug', 'products.badge',
                    'products.selling', 'products.offered', 'products.brand_id', 'products.updated_at',
                ...
                $query = $query->select('products.id', 'products.title', 'products.slug', 'products.badge',
                    'products.selling', 'products.offered', 'products.brand_id', 'products.updated_at',
                    'products.image', 'products.shipping_rule_id',
                    'products.admin_id',
                    'products.review_count', 'products.rating', 'flash_sale_products.price',
                    'flash_sales.end_time');
            }
        ...
        try {
            ...
            if ($lang) {
                if($request->first_load){
                    $banner = Banner::where('id', Config::get('constants.banner.BANNER_10'))->first();
                    $sliders = HomeSlider::where('status', Config::get('constants.status.PUBLIC'))
                        ->where('slider_type', Config::get('constants.sliderType.OFFER'))
                    ...
                    $offerMeta = OfferMeta::leftJoin('offer_meta_langs as cl', function ($join) use ($lang) {
                            $join->on('cl.offer_meta_id', '=', 'offer_metas.id');
                            $join->where('cl.lang', $lang);
                        })
                        ->select('offer_metas.*', 'cl.meta_title', 'cl.meta_description', 'cl.meta_keywords')
                        ->first();
                }
                ...
                $query = $query->select('products.id', 'pl.title', 'products.slug', 'pl.badge',
                    'products.selling', 'products.offered', 'products.brand_id', 'products.updated_at',
                    'products.image', 'products.review_count',
                    'products.shipping_rule_id',
                    'products.admin_id',
                    'products.rating', 'flash_sale_products.price',
                    'flash_sales.end_time',
                    'product_offers.status as product_is_active',
                    'product_offers.start_time as product_offer_start_time',
                    'product_offers.end_time as product_offer_end_time',
                    'brand_offers.status as brand_is_active',
                    'brand_offers.start_time as brand_offer_start_time',
                    'brand_offers.end_time as brand_offer_end_time',
                    'category_offers.status as category_is_active',
                    'category_offers.start_time as category_offer_start_time',
                    'category_offers.end_time as category_offer_end_time');
            ...
            $query = $query->leftJoin('offer_products', 'products.id', '=', 'offer_products.product_id')
                ->leftJoin('brands', 'products.brand_id', '=', 'brands.id')
                ->leftJoin('offer_brands', 'brands.id', '=', 'offer_brands.brand_id')
                ->leftJoin('product_categories', 'products.id', '=', 'product_categories.product_id')
                ->leftJoin('offer_categories', 'product_categories.category_id', '=', 'offer_categories.category_id')

                ->leftJoin('offers', function ($join) {
                    $join->on('offer_products.offer_id', '=', 'offers.id')
                        ->orOn('offer_brands.offer_id', '=', 'offers.id')
                        ->orOn('offer_categories.offer_id', '=', 'offers.id')
                        ->orOn('products.shipping_rule_id', '=', 'offers.shipping_rule_id')
                        ->orOn('products.admin_id', '=', 'offers.vendor_id');
                })
                ->where(function ($query) {
                    $query->whereNotNull('offer_products.offer_id')
                        ->orWhereNotNull('offer_brands.offer_id')
                        ->orWhereNotNull('offer_categories.offer_id')
                        ->orWhereNotNull('offer_categories.offer_id')
                        ->orWhereNotNull('offers.id');
                })

                ->leftJoin('offer_users', 'offers.id', '=', 'offer_users.offer_id')
                ...
                ->leftJoin('offers as product_offers', 'offer_products.offer_id', '=', 'product_offers.id') // Join for offered products
                ->leftJoin('offers as brand_offers', 'offer_brands.offer_id', '=', 'brand_offers.id') // Join for offered brands
                ->leftJoin('offers as category_offers', 'offer_categories.offer_id', '=', 'category_offers.id') // Join for offered categories
                ->leftJoin('offers as shipping_offers', 'products.shipping_rule_id', '=', 'offers.shipping_rule_id')
                ->leftJoin('offers as admin_offers', 'products.admin_id', '=', 'offers.vendor_id')
                ->where(function ($query) {
                    $query->where(function ($q) {
                        $q->whereDate('product_offers.start_time', '<=', now())
                            ->whereDate('product_offers.end_time', '>=', now())
                            ->where('product_offers.status', true);
                    })
                        ->orWhere(function ($q) {
                            $q->whereDate('brand_offers.start_time', '<=', now())
                                ->whereDate('brand_offers.end_time', '>=', now())
                                ->where('brand_offers.status', true);
                        })
                        ->orWhere(function ($q) {
                            $q->whereDate('category_offers.start_time', '<=', now())
                                ->whereDate('category_offers.end_time', '>=', now())
                                ->where('category_offers.status', true);
                        })
                        ->orWhere(function ($q) {
                            $q->whereDate('shipping_offers.start_time', '<=', now())
                                ->whereDate('shipping_offers.end_time', '>=', now())
                                ->where('shipping_offers.status', true);
                        })
                        ->orWhere(function ($q) {
                            $q->whereDate('admin_offers.start_time', '<=', now())
                                ->whereDate('admin_offers.end_time', '>=', now())
                                ->where('admin_offers.status', true);
                        });
                });
    ...
    public function paymentGateway(Request $request)
    {
        try {
            ...
            $data['offers'] = Offer::leftJoin('offer_payments as pl', function ($join) {
                    $join->on('pl.offer_id', '=', 'offers.id');
                })
                ->with('offer_schema')
                ->where('status', true)
    ...
    public function products(Request $request)
    {
        try {
            ...
            if ($lang) {
                ...
                $query = $query->select(['products.id', 'products.slug', 'pl.badge', 'pl.title',
                    'products.selling', 'products.offered', 'products.brand_id',
                    'products.shipping_rule_id',
                    'products.admin_id',
                    'products.image', 'products.review_count', 'products.rating', 'flash_sale_products.price',
                    'flash_sales.end_time']);
        ...
        try {
            ...
            $homeData = Utils::cacheRemember($cacheKey . $lang, function () use ($request, $lang) {
                ...
                if ($lang) {
                    ...
                    // SLIDER
                    $sliders = HomeSlider::where('status', Config::get('constants.status.PUBLIC'))
                        ->where('slider_type', Config::get('constants.sliderType.HOME'))
                        ->orWhere('slider_type', null)
                        ->leftJoin('home_slider_langs as cl', function ($join) use ($lang) {
                            $join->on('cl.home_slider_id', '=', 'home_sliders.id');
                            $join->where('cl.lang', $lang);
                        })
            ...
            if(!$lang) {
                // FLASH SALES
                $homeData['flash_sales'] = FlashSale::with(['products' => function ( $query) use ($request) {
                    $query->offset(0);
                    $query->limit(Config::get('constants.pagination.FRONTEND_SEARCH'));

                    $query->with(['product' => function ($query) use ($request) {

                        $query->leftJoin('flash_sales', function ($join) {
                            $join->on('products.id', '=', 'flash_sale_products.product_id');
                            $join->leftJoin('flash_sale_products', function ($join) {
                                $join->on('flash_sales.id', '=', 'flash_sale_products.flash_sale_id');
                            });
                        ...
                        $query->select('products.id', 'products.slug', 'products.selling', 'products.offered',
                            'products.image', 'products.review_count', 'products.rating', 'products.title',
                            'products.badge',
                            'products.shipping_rule_id',
                            'products.admin_id',
                            'products.brand_id', 'flash_sales.end_time', 'flash_sale_products.price');
                    }]);
                ...
                $homeData['collections'] = $collections->with([
                    'collection_products' => function ($query) use ($request, $totalCollectionProduct) {
                        $query->with(['item' => function ($query) use ($request){
                            ...
                            $query->select('products.id', 'products.title', 'products.badge',
                                'products.selling', 'products.offered', 'products.slug',
                                'products.image', 'products.review_count', 'products.rating',
                                'products.shipping_rule_id',
                                'products.admin_id',
                                'flash_sale_products.price', 'products.brand_id',
                                'flash_sales.end_time');
                        }]);

                        $query->take($totalCollectionProduct);
                        $query->orderBy('collection_with_products.product_id', 'DESC');
                    }])
                    ->where('status', Config::get('constants.status.PUBLIC'))
                    ->get();

                $homeData['collections']->each(function ($collection) {
                    $collection->setRelation('collection_products',
                        $collection->collection_products->take(Config::get('constants.homePagePagination.COLLECTION')));
                });

            } else {
                // FLASH SALES
                $homeData['flash_sales'] = FlashSale::with(['products' => function ($query) use ($request, $lang) {
                    $query->offset(0);
                    $query->limit(Config::get('constants.pagination.FRONTEND_SEARCH'));

                    $query->with(['product' => function ($query) use ($request, $lang) {

                        $query->leftJoin('product_langs as avl', function ($join) use ($lang) {
                        ...
                        $query->select('products.id', 'products.slug', 'products.selling', 'products.offered',
                            'products.image', 'products.review_count', 'products.rating', 'avl.title',
                            'products.shipping_rule_id',
                            'products.admin_id',
                            'avl.badge',
                            'products.brand_id', 'flash_sales.end_time', 'flash_sale_products.price');
                ...
                $homeData['collections'] = $collectionsQuery->with([
                    'collection_products' => function ($query) use ($request, $totalCollectionProduct, $lang) {

                        $query->with(['item' => function ($query) use ($request, $lang) {
                            ...
                            $query->select('products.id', 'avl.title', 'avl.badge',
                                'products.selling', 'products.offered', 'products.slug',
                                'products.image', 'products.review_count', 'products.rating',
                                'products.shipping_rule_id',
                                'products.admin_id',
    ...
    public function product(Request $request, $id)
    {
        try {
            ...
            $productData = Utils::cacheRemember($cacheKey, function () use ($request, $id, $lang) {
                ...
                if ($lang) {
                    ...
                    $query = $query->leftJoin('flash_sales', function ($join) {
                        ...
                        $join->where('flash_sales.start_time', '<=', date('Y-m-d H:i:s'))
                    ...
                    $query = $query->with(['brand' => function ($query) {
                        ...
                        ->with('bundle_deal')
                        ->with('product_image_names.attributes')
                        ->with(['shipping_rule' => function ($query) {
                        $query->with(['shipping_places']);
                    ...
                    $query = $query->select('products.*', 'flash_sale_products.price',
                        'flash_sales.end_time',
                        'user_wishlists.id as wishlisted');
                    ...
                    $primaryCategory = ProductCategory::with(['category' => function ($primaryCategory) {
                        $primaryCategory->with(['parent_data']);
                    }])
                        ->where('product_id', $id)
                        ->where('primary', Config::get('constants.status.PUBLIC'))
                        ->first();
            ...
            if($lang){
                $pq = Utils::offerQueryLang($request, $pq, $lang);
            } else {
                $pq = Utils::offerQuery($request, $pq);
            }
            $pq = $pq->select('id', 'admin_id', 'brand_id', 'shipping_rule_id');
            $data = $pq->find($id);
            $productData['offer'] = $data;
            ...
            if (!is_null($id)) {
                ...
                if (!is_null($flashSale)) {
                    ...
                    if ($lang) {
                        ...
                        $query = $query->with(['product' => function ($query) use ($request, $lang) {
                            $query->leftJoin('flash_sales', function ($join) use ($request) {
                            ...
                            $query->select('products.id', 'products.title',
                                'products.selling', 'products.offered', 'products.slug', 'products.brand_id',
                                'products.image', 'products.review_count', 'products.rating',
                                 'products.shipping_rule_id',
                                 'products.admin_id',
                                'avl.title', 'avl.badge',
                        ...
                        $query = $query->with(['product' => function ($query) use ($request) {
                            $query->leftJoin('flash_sales', function ($join)  use ($request) {
                            ...
                            $query = Utils::offerQuery($request, $query);
                            $query->where('products.status', Config::get('constants.status.PUBLIC'))
                                ->select('products.id', 'products.title', 'products.brand_id',
                                    'products.shipping_rule_id',
                                    'products.admin_id',
                                    'products.selling', 'products.offered', 'products.badge', 'products.slug',
                                    'products.image', 'products.review_count', 'products.rating',
                                    'flash_sale_products.price', 'flash_sales.end_time');
                        }]);
                ...
                if ($lang) {
                    ...
                    $flashSaleQuery = $flashSaleQuery->with(['products' => function ($query) use ($request, $lang) {
                        ...
                        $query->with(['product' => function ($query) use ($request, $lang){
                            ...
                            $query->where('products.status', Config::get('constants.status.PUBLIC'))
                                ->select('products.id', 'avl.title', 'products.brand_id',
                                    'products.shipping_rule_id',
                                    'products.admin_id',
                                    'products.selling', 'products.offered', 'avl.badge', 'products.slug',
                                    'products.image', 'products.review_count', 'products.rating',
                                    'flash_sale_products.price', 'flash_sales.end_time');
                        }]);
                    }]);
                    ...
                    $flashSaleQuery = $flashSaleQuery->with(['products' => function ($query) use ($request) {
    ...
    public function productSuggestion(Request $request, $id)
    {
        try {
            ...
            if ($product) {
                ...
                $qSug2 = Product::query();
                $qSug2 = $qSug2->join('product_categories as pc', 'pc.product_id', '=', 'products.id')
                ...
                if ($lang) {
                    ...
                    $qSug1 = Utils::offerQueryLang($request, $qSug1, $lang);
                    ...
                    $qSug2 = Utils::offerQueryLang($request, $qSug2, $lang);
                    ...
                    $qSug1 = Utils::offerQuery($request, $qSug1);
                    $qSug2 = Utils::offerQuery($request, $qSug2);
                ...
                if ($request->page == 1 && ($count1 > 0 || $count2 > 0)) {
                    ...
                    if ($lang) {
                        ...
                        $tempQuery = Utils::offerQueryLang($request, $tempQuery, $lang);
                        ...
                        $tempQuery = Utils::offerQuery($request, $tempQuery);
