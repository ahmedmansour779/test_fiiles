...
class AdminsController extends Controller
{
    ...
    public function allAdmins(Request $request)
    {
        try {
            ...
            if ($request->q) {
                $query = $query->where('admins.name', 'LIKE', "%{$request->q}%");
            }
            $query = $query->select('admins.id', 'admins.name');

            $query = $query->orderBy('admins.created_at');
            if($request->per_page) {
    ...
    public function delete(Request $request, $id)
    {
        try {
            ...
            Offer::where('vendor_id', $id)->update(['vendor_id' => null]);


            $offers = Offer::where('admin_id', $id)->get();
            foreach ($offers as $i) {
                OfferLang::where('offer_id', $i->id)->delete();
                Offer::where('id', $i->id)->delete();
            }
