    ...
    public function allUsers(Request $request)
    {
        try {
            $query = User::query();
                ...
                if ($request->q) {
                    $query = $query->where('users.name', 'LIKE', "%{$request->q}%");
                    $query = $query->orWhere('users.email', 'LIKE', "%{$request->q}%");
                }
    ...
    public function delete(Request $request, $id)
    {
        try {
            ...
            foreach ($ids as $i) {
                ...
                OfferUser::where('user_id', $i)->delete();
    ...
    public function login(Request $request)
    {
        try {
            ...
            if ($request->user_token) {
                ...
                // Moving offer guest user to logged in user
                $guestUser = GuestUser::where('user_token', $request->user_token)->first();
                $guestOffers = OfferGuestUser::where('guest_user_id', $guestUser->id)->get();

                foreach ($guestOffers as $i) {
                    $userOffer = OfferUser::where('user_id', $user->id)
                    ...
                    if(!$userOffer){
                        ...
                        OfferGuestUser::where('id', $i->id)->delete();
                    }
                }
    ...
    public function signup(Request $request)
    {
        try {
            ...
            if (!$existingUser) {
                ...
                User::where('email', $existingUser->email)
                    ->update([
                    'code' => $request['code'],
        ...
        try {
            ...
            if ($request->user('user')) {
                ...
                if($lang) {
                    ...
                    $query = $query->with(['offer_user' => function($query) use ($lang) {
                        $query->with(['offer' => function ($query) use ($lang) {
                            $query->leftJoin('offer_langs as ol', function ($join) use ($lang) {
                                ...
                                ->where('offers.start_time', '<=', date('Y-m-d H:i:s'))
                                ->where('offers.end_time', '>=', date('Y-m-d H:i:s'));;
                        }]);
                    }]);

                } else {
                    $query = $query->with(['offer_user' => function($query){
                ...
                $user= $query->where('id', $request->user('user')->id)
                    ->first();
                ...
                if($lang) {
                    $query = $query->with(['offer_guest_user' => function($query) use ($lang) {
                        $query->with(['offer' => function ($query) use ($lang) {
                            $query->leftJoin('offer_langs as ol', function ($join) use ($lang) {
                                $join->on('offers.id', '=', 'ol.offer_id');
                                $join->where('ol.lang', $lang);
                    ...
                    $query = $query->with(['offer_guest_user' => function($query) {
                        $query->with(['offer' => function ($query) {
                            $query->with(['offer_schema'])
                                ->where('offers.status', true)
                                ->where('offers.start_time', '<=', date('Y-m-d H:i:s'))
               ...
               $user = $query->where('user_token', $request->user_token)->first();
