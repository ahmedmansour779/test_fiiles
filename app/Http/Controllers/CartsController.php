    ...
    public function byUser(Request $request)
    {
        try {
            ...
            if ($lang) {
                ...
                $query = $query->with(['flash_product' => function ($query) use ($request, $userId, $guestUserId, $now, $lang) {
                    ...
                    $query->with(['shipping_rule' => function($query) use ($userId, $guestUserId, $now, $lang) {
                        ...
                        $query = Utils::offerCheckLangQuery($userId, $guestUserId, $query, $now, $lang);
                    ...
                    $query->leftJoin('product_langs as pl', function ($join) use ($lang) {
                        ...
                        ->select('products.id', 'products.bundle_deal_id', 'pl.title', 'products.slug',
                            ...
                            'products.brand_id',
                            'products.admin_id',
                            'products.image', 'products.shipping_rule_id', 'products.review_count', 'products.rating', 'products.shipping_rule_id',
                ...
                $query = $query->with(['flash_product' => function ($query) use ($request, $userId, $guestUserId, $now) {
                    ...
                    $query->with(['shipping_rule' => function ($query) use($userId, $guestUserId, $now) {
                        $query = Utils::offerCheckQuery($userId, $guestUserId, $query, $now);
                        $query->with(['shipping_places' => function($query){
                            $query->with(['shipping_rule']);
                        }]);
                    }]);
            ...
            if($this->user && !$request->user_token){
    ...
    public function updateShipping(Request $request)
    {
        try {
            ...
            $query = $query->with(['flash_product' => function($query) use ($request, $userId, $guestUserId, $now) {
                ...
                $query->with(['shipping_rule' => function ($query) use ($request, $userId, $guestUserId, $now) {
                    $query = Utils::offerCheckQuery($userId, $guestUserId, $query, $now);
