...
class OrdersController extends ControllerHelper
{
                    ...
                    $query = $query->with(['ordered_products' => function ($query) use ($lang) {
                        ...
                        $query->with(['product' => function($query) use ($lang){
                            ...
                            $query->leftJoin('product_langs as pl', function ($join) use ($lang) {
                                ...
                                ->select(['products.id', 'pl.title', 'products.slug',
                                    'products.image', 'products.selling',
                                    'products.offered', 'products.shipping_rule_id',
                                    'products.bundle_deal_id', 'pl.unit']);
                ...
                if ($lang) {
                    $query = $query->with(['ordered_products.product' => function ($query) use ($lang) {
                        ...
                        $query->leftJoin('product_langs as pl', function ($join) use ($lang) {
                            ...
                            ->select('products.id', 'pl.title', 'products.slug', 'products.image',
                                'products.selling',
                                'products.offered', 'products.shipping_rule_id',
                                'products.bundle_deal_id', 'pl.unit');
                    }]);
    ...
    public function action(Request $request)
    {
        try {
            ...
            $paymentOffer = Offer::join('offer_payments as pl', function ($join) use ($request) {
                $join->on('pl.offer_id', '=', 'offers.id')
                    ->where('payment_method_id', $request->order_method);
            })
                ->with('offer_schema')
                ->where('status', true)
            ...
            $cartQuery = Cart::with(['product_inner' => function($query) use ($request) {
            ...
            foreach ($existingCart as $key => $cart) {
                if ($cart->shipping_place_id && !is_null($cart->product_inner)) {
                    ...
                    $offer = Utils::getCurrentOffer($cart->product_inner);

                    if(!$offer && $paymentOffer) {
                        $offer = $paymentOffer;
                    }

                    $discount += Utils::discountedPrice($cart->quantity, $currentPrice, $offer);
            ...
            if (count($existingCart) > 0) {
                ...
                foreach ($existingCart as $key => $cart) {
                    ...
                    if ($cart->shipping_place_id && !is_null($cart->product_inner)) {
                        ...
                        if ($taxRule) {
                            if ((int)$taxRule->type === (int)Config::get('constants.priceType.FLAT')) {
                                ...
                                $taxQtyOffer = number_format(
                                    (float)($taxRule->price * ($priceWithoutBundle - $discount)) / 100,
                                    2, '.', '');
                            }
                        }
                        ...
                        $totalTax = (float)$taxQtyOffer;

                        $total = (float)($shippingPrice + $totalTax + $priceWithoutBundle - $discount);
                        ...
                        UpdatedInventory::where('id', $cart->inventory_id)
