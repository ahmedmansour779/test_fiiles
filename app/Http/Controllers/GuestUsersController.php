...
class GuestUsersController extends ControllerHelper
{
    ...
    public function allGuestUsers(Request $request)
    {
        try {
            ...
            if ($request->q) {
                $query = $query->where('guest_users.name', 'LIKE', "%{$request->q}%");
                $query = $query->orWhere('guest_users.email', 'LIKE', "%{$request->q}%");
            }
            $query = $query->select('guest_users.id', 'guest_users.name');


            $query = $query->orderBy('guest_users.created_at');
            if($request->per_page) {
    ...
    public function delete(Request $request, $id)
    {
        try {
            ...
            foreach ($ids as $i) {
                ...
                OfferGuestUser::where('guest_user_id', $i)->delete();
